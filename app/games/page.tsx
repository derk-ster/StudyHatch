    ...
    const socket = createGameSocket({
      onMessage: (message) => {
        ...
        if (message.type === 'session_joined') {
          ...
          socket.close();
          if (session?.userId) {
            recordStudentActivityForClasses(session.userId, 'game_hosted', `Hosted game ${message.payload.code}.`);
          }
    ...
    const socket = createGameSocket({
      onMessage: (message) => {
        ...
        if (message.type === 'session_joined') {
          ...
          socket.close();
          if (session?.userId) {
            recordStudentActivityForClasses(session.userId, 'game_joined', `Joined game ${message.payload.code}.`);
          }
          router.push(`/games/lobby/${message.payload.code}`);
        }
      },
